  ...
  (function($) {
    return $.bigfoot = function(options) {
      ...
      defaults = {
        ...
        buttonMarkup: "<div class='bigfoot-footnote__container'> <div class='bigfoot-footnote__button' id='{{SUP:data-footnote-backlink-ref}}' data-footnote-number='{{FOOTNOTENUM}}' data-footnote-identifier='{{FOOTNOTEID}}' alt='See Footnote {{FOOTNOTENUM}}' rel='footnote' data-bigfoot-footnote='{{FOOTNOTECONTENT}}'>{{BUTTON_HTML}}</div> </div>"
      };
      ...
      footnoteInit = function() {
        ...
        footnoteButtonSearchQuery = settings.scope ? "" + settings.scope + " span[href*='#']" : "span[href*='#']";
        $footnoteAnchors = $(footnoteButtonSearchQuery).filter(function() {
          ...
          return ("" + ($this.attr("href")) + relAttr).match(settings.anchorPattern) && $this.closest("[class*=" + settings.footnoteParentClass + "]:not(span):not(" + settings.anchorParentTagname + ")").length < 1;
        });
        footnotes = [];
        footnoteLinks = [];
        finalFNLinks = [];
        ...
        for (i = _i = 0, _ref = footnotes.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          ...
          if (footnoteContent.indexOf("<") !== 0) {
            footnoteContent = "<div>" + footnoteContent + "</div>";
          }
          footnoteButton = settings.buttonMarkup.replace(/\{\{FOOTNOTENUM\}\}/g, footnoteNum).replace(/\{\{FOOTNOTEID\}\}/g, footnoteIDNum).replace(/\{\{FOOTNOTECONTENT\}\}/g, footnoteContent).replace(/\{\{BUTTON_HTML\}\}/g, $relevantFNLink.html());
